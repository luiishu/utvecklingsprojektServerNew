...
pub fn handle_get_request_new(request: &String, conn: &Connection) -> String {
    ...
    println!(
        "Request line received from struct: {}",
    ...
    println!(
        "Request URI file type from struct: {}",
        request_line.uri_file_type
    );

    // 4. Check if file exists in server
    if Path::new(&request_line.get_request_uri_without_first_slash()).exists() {
        println!(
            "File path for uri {} exists!",
            &request_line.get_request_uri_without_first_slash()
        );
    } else {
    ...
    let (status_line, filename) =
        if Path::new(&request_line.get_request_uri_without_first_slash()).exists() {
            ...
            if &request_line.to_string() == "GET / HTTP/1.1" {
                ...
                let data_vector =
                    get_query_iterator(conn, "SELECT * FROM product ORDER BY id DESC LIMIT 1;");
                ...
                data = format!(
                    "{:?}",
                    get_query_iterator(conn, "SELECT * FROM [order] ORDER BY id DESC LIMIT 1;")
                );
...
pub fn contains_resource(request_header: &str) -> bool {
    request_header.contains(".png")
        || request_header.contains(".jpg")
        || request_header.contains(".jpeg")
}
